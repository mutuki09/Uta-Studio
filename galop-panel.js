void async function(){
 ...
 try{const response=await fetch('/galop/',{cache:'no-store'});if(!response.ok)return;}catch{return;}
 const old=document.getElementById('mvPanel');old.hidden=true;
 const section=document.createElement('section');section.id='galopPanel';section.style.cssText='max-width:1400px;margin:24px auto;padding:24px;background:#fffaf7;border-radius:24px';
 section.innerHTML='<h2>⑥ MV制作 · GalOP</h2><p>8系統の演出・背景・色調整・構成シャッフル・MP4出力</p><button id="sendGalop">今の曲をMVに引き継ぐ</button> <button id="reloadGalop">制作画面を再接続</button><p id="galopMessage" role="status">GalOP Web Studio（4195）の起動が必要です。画像を選び、20秒プレビューから確認してください。</p><iframe title="GalOP MV制作" src="/galop/" style="width:100%;height:1000px;border:0;border-radius:16px"></iframe>';
 old.after(section);const frame=section.querySelector('iframe'),message=section.querySelector('#galopMessage');
