...
let token='';
export async function galopProxy(req,res,frontendPort=4193){
 if(!req.url.startsWith('/galop/'))return false;
 ...
 const route=req.url.slice(6);
 if(!/^\/(?:$|upload$|render$|status$|asset\/|result\/)/.test(route)){res.writeHead(404);res.end();return true;}
 try{
 if(!token||route==='/'){
 const page=await fetch('http://127.0.0.1:4195/');if(!page.ok)throw Error('GalOPを起動してください');
 let html=await page.text();token=html.match(/const token='([^']+)'/)?.[1];if(!token)throw Error('GalOP認証情報を取得できません');
 if(route==='/'){
 html=html.replace("fetch(url,","fetch('/galop'+url,").replaceAll("'/asset/","'/galop/asset/")
 .replace("$('video').src=s.url;","$('video').src='/galop'+s.url;").replace("$('download').href=s.url;","$('download').href='/galop'+s.url;")
 ...
 const headers={'host':'127.0.0.1:4195','origin':'http://127.0.0.1:4195','x-galop-token':token};
 ...
 upstream.on('error',()=>{if(!res.headersSent)res.writeHead(502);res.end('GalOP Web Studio (4195) を起動してください');});req.pipe(upstream);
 }catch(e){res.writeHead(502,{'Content-Type':'text/plain; charset=utf-8'});res.end('GalOP Web Studio (4195) を起動してください。'+e.message);}
 return true;
}
