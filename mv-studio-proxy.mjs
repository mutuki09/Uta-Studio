...
export async function mvStudioProxy(req,res,frontendPort=4193){
 ...
 html=html.replace('</html>',`<script>window.addEventListener('message',async e=>{if(e.origin!==location.origin||e.source!==parent)return;if(e.data?.type==='show-render-controls'){$('preview').scrollIntoView({behavior:'smooth',block:'center'});$('preview').focus();return;}if(e.data?.type==='song-transfer'){try{if(uploading)throw Error('素材の読み込み完了を待ってください');const d=await api('/upload',{method:'POST',headers:{'X-File-Name':'song.wav'},body:e.data.audio});audio=d.src;$('audioName').textContent='song.wav（現在の曲）';$('bpm').value=e.data.bpm;update();parent.postMessage({type:'song-transfer-result',ok:true},location.origin);}catch(err){parent.postMessage({type:'song-transfer-result',ok:false,error:err.message},location.origin);}return;}if(e.data?.type==='asset-pack-transfer'||e.data?.type==='cc0-pack-transfer'){const resultType=e.data.type==='asset-pack-transfer'?'asset-pack-transfer-result':'cc0-pack-transfer-result';const label=e.data.label||'CC0素材';try{if(uploading)throw Error('素材の読み込み完了を待ってください');const upload=async item=>{uploading++;try{const d=await api('/upload',{method:'POST',headers:{'X-File-Name':item.name},body:item.blob});return {src:d.src,name:item.name,fit:item.fit};}finally{uploading--;}};const [nextMedia,nextBackgrounds]=await Promise.all([Promise.all(e.data.main.map(upload)),Promise.all(e.data.backgrounds.map(upload))]);media=nextMedia;backgrounds=nextBackgrounds;if(e.data.preset&&[...$('preset').options].some(option=>option.value===e.data.preset))$('preset').value=e.data.preset;refresh();update();const count=media.length+backgrounds.length;$('status').textContent=label+count+'枚を読み込みました。下の「20秒プレビュー」で映像を作れます';parent.postMessage({type:resultType,ok:true,count,label},location.origin);}catch(err){parent.postMessage({type:resultType,ok:false,error:err.message,label},location.origin);}}});</script></html>`);
